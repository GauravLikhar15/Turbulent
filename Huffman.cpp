...
void huffmanCoding(char str[],int size){
   priority_queue<node> qu;
   int frequency[256];
   for(int i = 0; i<256; i++){frequency[i] = 0;}
   for(int i = 0; i <size; i++){frequency[int(str[i])]++;}   //countig the freq
   for(int i = 0; i<256; i++){if(frequency[i]){qu.push(node(i, frequency[i]));}}
   while(qu.size() >1){
